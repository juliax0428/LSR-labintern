'''Isochone File generated by CMD 3.7 (http://stev.oapd.inaf.it/cmd)'''
'''
Python code for 
    1. Read isochrones in (linear age / log age) 
    2. Explore isochrones trend through changing Av and metallicites
'''
...
def iso_changing_metallicities():
    meta_values = [-2, -1.5, -1, 0.1, 0.5]
    meta_files = {m: f'/Users/xxz/Desktop/LSR-labintern/iso_changing_meta/iso_meta_{m}.dat' for m in meta_values}
    isochrone_data = {key: readCDSTable(file) for key, file in meta_files.items()}

    unique_logAges = np.unique(isochrone_data[-2]['logAge'])

    for age in unique_logAges:
        fig, ax = plt.subplots()
        
        for m, df in isochrone_data.items():
            # Filter data by age
            new_df = df[np.isclose(df['logAge'], age)]
            if len(new_df) == 0:
                continue
            g_mag = new_df['Gmag']
            bp_rp = new_df['G_BPmag'] - new_df['G_RPmag']
            ax.scatter(bp_rp, g_mag, label=f'metallicity={m}, age={age}', s=2, marker=',')
        
        # Plotting settings
        ax.invert_yaxis()
        ax.set_xlabel('G_BP - G_RP')
        ax.set_ylabel('G Magnitude')
        ax.set_title(f'Isochrones for changing metallicities at age{age}')
        ax.legend(loc='best')
        plt.show()
